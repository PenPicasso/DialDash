...
function rank(originalNode: NodeData) {
  ...
  return {
    ...
    email: solResolution?.email || node.email,
    bestOutreachChannel: solResolution?.bestOutreachChannel || node.bestOutreachChannel,
    reachabilityStatus: solResolution?.reachabilityStatus || node.reachabilityStatus,
    publishingCadence: solResolution?.publishingCadence || node.publishingCadence,
    cadenceConfidence: solResolution?.cadenceConfidence || node.cadenceConfidence,
    frequencyEpisodesPerMonth: solResolution?.frequencyEpisodesPerMonth || node.frequencyEpisodesPerMonth,
