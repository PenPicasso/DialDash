...
for (let batch = 1; batch <= batchCount; batch += 1) {
  ...
  expectedCount += manifest.records?.length || 0;
  records.push(...result.records);
}
if (records.length !== expectedCount) throw new Error(`Expected ${expectedCount} Terra Medium records, found ${records.length}.`);
if (records.some((record) => !record.id) || new Set(records.map((record) => record.id)).size !== expectedCount) {
