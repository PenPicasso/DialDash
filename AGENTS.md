...
- Batches complete: 8 of 8 (179 of 179 records).
- Decisions: 3 `PURSUE_NOW`, 51 `NURTURE`, 125 `DISQUALIFIED_CONFIRMED`.
...
- Batches 5-6 are dominated by official RSS/Apple feeds with publications outside the 90-day activity window; these are inactivity exclusions, not missing-contact or missing-offer exclusions.
- The recomposed 200-record cohort passed Sol review: 200 accepted decisions, zero returned drafts, and 40/40 deterministic audit verdicts correct (100% precision against a 90% gate).
- Sol audit verdicts are durable at `data/sol-next-200-b-audit-verdicts.json`; `scripts/review-terra-medium-cohort.ts` records a passing summary in the composed cohort while production remains unchanged.
...
- Sol's adversarial review later found that those passes only proved schema completeness: all 179 NURTURE rows are automated drafts, not completed prospect research. `validate-terra-medium-next-200.ts` now fails those rows unless `researchCompleteness` is `COMPLETE`; `--allow-draft` is staging-only. Feed freshness can support owned long-form only after the feed/listing identity strongly matches the prospect; an Apple-to-RSS binding alone is insufficient.
- Run `npx tsx scripts/review-terra-medium-cohort.ts --cohort=next-200-b` for the fail-closed Sol gate. Its ignored report is `storage/sol-next-200-b/review.json`. The current result is FAILED: 21 factual exclusions accepted, 179 returned to research, and only 3 of the deterministic 40-record audit sample have explicit Sol verdicts. Do not start another cohort until every returned record is manually completed and the full sample reaches at least 90% audited precision.
- The 179 recovery queue is prepared at `storage/terra-redo-next-200-b/` as eight Terra batches of 25/25/25/25/25/25/25/4. The authoritative execution contract is `docs/TERRA_REDO_179.md`. Every `researchCompleteness: COMPLETE` record must include an evidence-backed `researchAudit` for owned long-form, cadence, roles, contact, offer, funnel, video gap, and pitch hook; the batch validator enforces this. Preserve the 21 previously accepted factual exclusions in their original source batches.
...
- `data/full-review.json` now carries a `commercial-intelligence-v2` layer for all 840 prospects. It is additive and must never alter funnel strategy, outbound score, signability, outreach, or review decisions.
- Commercial Intelligence identifies the primary revenue engine, ranks up to three revenue streams, explains content's role in the transaction, frames revenue leverage and a value conversation, and attaches separate economics to every revenue-stack item. Pricing is labeled `VERIFIED`, `ESTIMATED`, or `INFERRED`, with the public filing, government rate card, or first-party price used to construct the range.
- `lib/commercialBenchmarks.ts` is the benchmark registry for research, advisory, board, investment, sponsorship, speaking, training, data/software, recruiting, enterprise services, audience support, intellectual property, and institutional mandates. It also contains the eight-category avatar guide and exact prospect-level public-price overrides.
- Every sourced benchmark includes named people to study plus an explicit `priceConnection` explaining whether the person publishes the price, a buyer disclosed an observed transaction, or a separate public comparable supplies the estimate. Do not imply that a market comparable is the named person's quote.
- Report images use direct unoptimized static paths because the lightweight Sites router does not expose Next.js's dynamic image-optimizer route. Keep `unoptimized` on `/report` images unless the Sites runtime changes.
- `npm run build` now builds Next directly and packages `public/`, `.next/static`, and prerendered app HTML into the existing lightweight Sites artifact. Do not reintroduce the full OpenNext conversion unless a real server-only route requires it.
...
- Rory Johnston and Arjun Murti are evidence-specific exemplars, not category templates: Rory's official paid research price is verified while advisory and speaking use sourced estimates; Arjun's advisory engagement and public-board programmes use SEC-disclosed economics while Veriten's investment terms remain estimated.
